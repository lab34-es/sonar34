 ...
 * We reconstruct what the file looked like before and after the change.
 */
function buildDiffTexts(file) {
  // If no hunks, show raw diff in modified only
  ...
  for (const hunk of file.hunks) {
    ...
    // Add a visual separator line for context
    originalLines.push(`\u200B`) // zero-width space as blank separator
    modifiedLines.push(`\u200B`)
...
/**
 * Derive a Monaco language from file extension.
 */
...
export default function CommitDiffModal({ open, onClose, repo, sha }) {
  ...
  const fetchDiff = useCallback(async () => {
    ...
    try {
      // repo is like "workspace/slug"
      const [workspace, slug] = repo.split('/')
      const res = await fetch(
        `${SOCKET_URL}/api/admin/repos/${encodeURIComponent(workspace)}/${encodeURIComponent(slug)}/commits/${sha}/diff`
      )
      if (!res.ok) {
